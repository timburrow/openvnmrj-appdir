...
# go into contribution directory
  mkdir "${gitdir}"/"${archivename}"
  cd "${gitdir}"/"${archivename}"
  tar xf "${_PATH}/${archive}"
...
To have several contributions, make a new branch and cherry-pick the tags:  
...
#  archivename=`basename ${archive} .tar.Z`
  description=""
  _description "${archivename}"/"${archivename}.README"
# make a tar file and delete the directory
  cd "${archivename}"
  tar zcf ../"${archivename}".tar.Z *
  cd ..
  cp "${archivename}/${archivename}.README" .
  rm -r "${archivename}"
  git checkout ${gitcheckout} "${archivename}"
  # clear all files, just have our orphan
  git rm -rf .
  git add "${archivename}".tar.Z
